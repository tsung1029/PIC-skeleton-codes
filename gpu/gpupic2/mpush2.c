...
#pragma omp target
#pragma omp parallel for \
private(i,j,k,noff,moff,npp,npoff,nn,mm,x,y,dxp,dyp,amx,amy,dx,dy, \
vx,vy,sum1,sfxy) \
reduction(+:sum2)
   for (k = 0; k < mxy1; k++) {
      noff = k/mx1;
...
#pragma omp target
#pragma omp parallel for \
private(i,j,k,l,ns,ns2,km,kmr,k1,k2,jj,j1,j2,joff,at1,ani,t1,t2,t3)
   for (i = nyi-1; i < nyt; i++) {
      joff = 2*nxhd*i;
...
#pragma omp target
#pragma omp parallel for \
private(i,j,k,l,ns,ns2,km,kmr,k1,k2,jj,j1,j2,joff,at1,t1,t2,t3)
   for (i = nyi-1; i < nyt; i++) {
      joff = 2*nxhd*i;
/* scramble coefficients */
      kmr = nxy/nx;
...
/* inverse fourier transform */
   if (isign < 0) {
/* perform x fft */
      cfft2rm2x(f,isign,mixup,sct,indx,indy,nyi,ny,nxhd,nyd,nxhyd,
                nxyhd);
/* perform y fft */
...
/*--------------------------------------------------------------------*/
void cgppush2l_(float *ppart, float *fxy, int *kpic, float *qbm,
                float *dt, float *ek, int *idimp, int *nppmx, int *nx,
                int *ny, int *mx, int *my, int *nxv, int *nyv, int *mx1,
                int *mxy1, int *ipbc) {
   cgppush2l(ppart,fxy,kpic,*qbm,*dt,ek,*idimp,*nppmx,*nx,*ny,*mx,*my,
             *nxv,*nyv,*mx1,*mxy1,*ipbc);
   return;
}

/*--------------------------------------------------------------------*/
void cgppushf2l_(float *ppart, float *fxy, int *kpic, int *ncl,
                 int *ihole, float *qbm, float *dt, float *ek,
                 int *idimp, int *nppmx, int *nx, int *ny, int *mx,
                 int *my, int *nxv, int *nyv, int *mx1, int *mxy1,
                 int *ntmax, int *irc) {
   cgppushf2l(ppart,fxy,kpic,ncl,ihole,*qbm,*dt,ek,*idimp,*nppmx,*nx,
...
/*--------------------------------------------------------------------*/
void cmpois22_(float complex *q, float complex *fxy, int *isign,
               float complex *ffc, float *ax, float *ay, float *affp,
               float *we, int *nx, int *ny, int *nxvh, int *nyv,
               int *nxhd, int *nyhd) {
   cmpois22(q,fxy,*isign,ffc,*ax,*ay,*affp,we,*nx,*ny,*nxvh,*nyv,*nxhd,
            *nyhd);
   return;
}
...
/*--------------------------------------------------------------------*/
void cfft2rm2x_(float complex *f, int *isign, int *mixup,
                float complex *sct, int *indx, int *indy, int *nyi,
                int *nyp, int *nxhd, int *nyd, int *nxhyd, int *nxyhd) {
   cfft2rm2x(f,*isign,mixup,sct,*indx,*indy,*nyi,*nyp,*nxhd,*nyd,*nxhyd,
             *nxyhd);
   return;
}
...
/*--------------------------------------------------------------------*/
void cwfft2rm2_(float complex *f, int *isign, int *mixup,
                float complex *sct, int *indx, int *indy, int *nxhd,
                int *nyd, int *nxhyd, int *nxyhd) {
   cwfft2rm2(f,*isign,mixup,sct,*indx,*indy,*nxhd,*nyd,*nxhyd,*nxyhd);
   return;
}
