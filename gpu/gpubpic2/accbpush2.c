...
/*--------------------------------------------------------------------*/
void _cdistr2h(float part[], float vtx, float vty, float vtz, float vdx,
...
reduction(+:sum2)
#pragma acc parallel create(sfxy,sbxy) \
deviceptr(ppart,fxy,bxy,kpic,ek)
{
#pragma acc cache(sfxy,sbxy) //does this parallelization work for sbxy?
#pragma acc loop \
private(i,j,k,noff,moff,npp,npoff,nn,mm,x,y,dxp,dyp,amx,amy,dx,dy,vx,vy,sum1,sfxy,sbxy)
	   for (k = 0; k < mxy1; k++) {
		   noff = k / mx1;
		   moff = my * noff;
		   noff = mx * (k - mx1 * noff);
...
/*--------------------------------------------------------------------*/
void cgpupois23(float complex q[], float complex fxy[], int isign,
...
private(j,k,k1,jj,kk,jk,kj,jk2,dkx,dky,at1,at2,at3,zt1,zt2,wp,at4) \
deviceptr(qt,fxy,ffct,we)
   for (k = 0; k < nyh; k++) {
      dky = dny*(float) k;
      kk = nxhd*k;
      at1 = dky*dky;
...
deviceptr(qt,fxy,ffct,we) present(sum1)
	   {
...
/*--------------------------------------------------------------------*/
void cgpuibpois23(float complex cu[], float complex bxy[],
...
reduction(+:sum1)
#pragma acc parallel loop \
private(j,k,k1,jj,kk,kj,jk,jk2,dkx,dky,at1,at2,at3,zt1,zt2,wp) \
...
#pragma acc parallel vector_length(16) \
private(j,k,k1,jj,jk,jk2,dkx,at1,at2,at3,zt1,zt2,wp) \
deviceptr(qt,bxy,ffct,we) present(sum1)
		{
	   for (j = 1; j < nxh; j++) {
...
#pragma acc serial \
deviceptr(wm,bxy) present(sum1) private(nxh1,k1)
		{
	   bxy[0] = zero;
	   bxy[1] = zero;
	   bxy[2] = zero;
...
/*--------------------------------------------------------------------*/
void cgpumaxwel2(float complex exy[], float complex bxy[],
...
/*--------------------------------------------------------------------*/
void cgpuemfield2(float complex fxy[], float complex exy[],
...
/*--------------------------------------------------------------------*/
void _cdistr2h_(float *part, float *vtx, float *vty, float *vtz,
...
/*--------------------------------------------------------------------*/
void cgpupois23_(float complex *q, float complex *fxy, int *isign,
                float complex *ffc, float *ax, float *ay, float *affp,
                float *we, int *nx, int *ny, int *nxvh, int *nyv,
                int *nxhd, int *nyhd) {
   cgpupois23(q,fxy,*isign,ffc,*ax,*ay,*affp,we,*nx,*ny,*nxvh,*nyv,
             *nxhd,*nyhd);
   return;
}
...
/*--------------------------------------------------------------------*/
void cgpuibpois23_(float complex *cu, float complex *bxy,
                  float complex *ffc, float *ci, float *wm, int *nx,
                  int *ny, int *nxvh, int *nyv, int *nxhd, int *nyhd) {
   cgpuibpois23(cu,bxy,ffc,*ci,wm,*nx,*ny,*nxvh,*nyv,*nxhd,*nyhd);
   return;
}

/*--------------------------------------------------------------------*/
void cgpumaxwel2_(float complex *exy, float complex *bxy,
                 float complex *cu, float complex *ffc, float *ci,
                 float *dt, float *wf, float *wm, int *nx, int *ny,
                 int *nxvh, int *nyv, int *nxhd, int *nyhd) {
   cgpumaxwel2(exy,bxy,cu,ffc,*ci,*dt,wf,wm,*nx,*ny,*nxvh,*nyv,*nxhd,
              *nyhd);
   return;
}

/*--------------------------------------------------------------------*/
void cgpuemfield2_(float complex *fxy, float complex *exy,
                  float complex *ffc, int *isign, int *nx, int *ny,
                  int *nxvh, int *nyv, int *nxhd, int *nyhd) {
   cgpuemfield2(fxy,exy,ffc,*isign,*nx,*ny,*nxvh,*nyv,*nxhd,*nyhd);
   return;
}
